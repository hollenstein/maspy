...
class ProteinInference(object):
    ...
    :ivar pepToProts: dict, for each peptide (=key) contains a set of parent
        proteins (=value). For Example {peptide: {protein, ...}, ...}
    :ivar proteins: dict, protein ids pointing to Protein objects
    ...
    :ivar clusters: dict, cluster ids pointing to a list of protein group ids
        that are part of the same cluster. See "Cluster identifier (MS:1002407)"

    :ivar _proteinToGroupIds: dict, protein ids containing a set of protein
        group ids that they are associated with.
        NOTE: Protein objects do not contain a reference to their protein groups
    :ivar _proteinToClusterId: dict, protein ids containing a set of cluster ids
        that they are associated with.
        NOTE: Protein objects do not contain a reference to their clusters
    :ivar _uniquePeptides: set, all unique peptides of all protein groups.
        I.e. peptides that are only mapped to one protein (after merging
        same-set proteins).
    :ivar _groupUniquePeptides: set, all group unique peptides of all protein
        groups. I.e. peptides that are not unique but only explained by proteins
        of one single protein group and not by any subsumable proteins.
    :ivar _groupSubsumablePeptides: set, all group subsumable peptides of all
        protein groups. I.e. peptides that are only explained by proteins of one
        single protein group but also of subsumable proteins.
    :ivar _sharedPeptides: set, all shared peptides of all protein groups.
        I.e. peptides that are explained by proteins of multiple protein groups,
        not considering subsumable proteins.
    """
...
class ProteinGroup(object):
    ...
    :ivar subset: set, sub-set proteins
    :ivar subsumableProteins: set, group sub-sumable proteins
    """
...
class Protein(object):
    """Protein represents one entry from a protein database. It describes the
